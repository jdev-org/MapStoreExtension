...
export function tabouLoadIdentifyContent(action$, store) {
    return action$
        .ofType(TABOU_CHANGE_FEATURES)
        .filter(() => isTabou2Activate(store.getState()))
        .switchMap((action) => {
            const resp = action.data;
            const respNotEmpty = keys(resp).filter(
                (k) => get(resp, k).data.features.length
            );
                ...
                .switchMap((response) => {
                        ...
                        let nomOA =
                            action.layer === "layerPA"
                                ? feature.nomopa
                                      ...
                                      .join("")
                                : feature.nom;
                        let name = `FicheSuivi_${action.id}_${feature.code}_${nomOA}`;
                        downloadToBlob(
                            response,
                        ...
                        return Rx.Observable.of(
                            success({
                                title: getMessageById(
                                    messages,
                                    "tabou2.infos.successApi"
                                ),
                                message: getMessageById(
                                    messages,
                                    "tabou2.infos.successPrint"
                                ),
                            })
                        );
                    }
                    return Rx.Observable.of(
                        error({
                            title: getMessageById(
                                messages,
                                "tabou2.infos.failApi"
                            ),
                            message: getMessageById(
                                messages,
                                "tabou2.infos.failPrint"
                            ),
                        })
                    );
                });
...
export function createChangeFeature(action$, store) {
    return action$.ofType(CHANGE_FEATURE).switchMap((action) => {
        let messages = store.getState()?.locale.messages;
        let layersToc = find(
            getPluginCfg(store.getState()).layersCfg,
            (o, i) => i === action.params.layer
        );
        return Rx.Observable.defer(() =>
            putRequestApi(
                `${get(URL_ADD, action.params.layer)}`,
                getPluginCfg(store.getState()).apiCfg,
...
export function displayFeatureInfos(action$, store) {
    return action$.ofType(DISPLAY_FEATURE).switchMap((action) => {
        // only if user create tabou feature from clicked map feature
        if (!action?.infos || !action.infos?.feature)
            return Rx.Observable.isEmpty();

        let tocLayer = layersSelector(store.getState()).filter(
            (lyr) => lyr.name === action.infos.layer
        )[0];
        ...
        return Rx.Observable.defer(() =>
            ...
            .switchMap((response) => {
                ...
                let selected = !isEmpty(getSelection(store.getState()))
                    ? getSelection(store.getState())?.feature
                    : null;
                let isSelected =
                    selected &&
                    selected.properties?.id_tabou ===
                        response.features[0]?.properties?.id_tabou;
                ...
                if (!isSelected) {
                    return Rx.Observable.of(
                        ...
                        updateFeatureInfoClickPoint(
                            clickPointSelector(store.getState())
                        )
                    );
                }
...
export function dipslayPASAByOperation(action$, store) {
    return action$.ofType(DISPLAY_PA_SA_BY_OA).switchMap(() => {
        const idTabou = getSelection(store.getState()).id;
        const idsPA = store
            .getState()
            .tabou2.ficheInfos.programmes.elements.map((p) => p.id);
        ...
        // SA filter
        let crossLayerFilter = getNewCrossLayerFilter(
            "INTERSECTS",
            "the_geom",
            "",
            [],
            ...
            return Rx.Observable.from([
                ...
                .map((r) =>
                    Rx.Observable.defer(() => r.api(id))
                        .catch((e) => {
                            console.log(
                                "Error on get getTabouVocationsInfos data request"
                            );
                            console.log(e);
